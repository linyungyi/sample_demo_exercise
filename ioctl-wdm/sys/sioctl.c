...
Abstract:
    ...
    Purpose of this driver is to demonstrate how read and write register by IOCTL.
...

{
    ...
    PCHAR               result4buffer = "From Device Driver Result";
    size_t              datalen4buffer = strlen(result4buffer)+1;//Length of data including null
    ...
    {
    case IOCTL_SIOCTL_READ_REGISTER:
        ...
        SIOCTL_KDPRINT(("IOCTL_SIOCTL_READ_REGISTER\n"));
        PrintIrpInfo(Irp);
        ...
        SIOCTL_KDPRINT(("\tData to User : (result,data)"));
        PrintChars(outBuf, 1);
    ...
    case IOCTL_SIOCTL_WRITE_REGISTER:
		...
		SIOCTL_KDPRINT(("IOCTL_SIOCTL_WRITE_REGISTER\n"));
		PrintIrpInfo(Irp);
		...
		SIOCTL_KDPRINT(("\tData to User : (result)"));
		PrintChars(outBuf, 1);
    ...
    case IOCTL_SIOCTL_INTERRUPT:
		...
		SIOCTL_KDPRINT(("IOCTL_SIOCTL_INTERRUPT\n"));
		PrintIrpInfo(Irp);
		...
		/*  write i2c here  */
		if (inBuf[0])
			regContext = regContext | 0x80;
		else
			regContext = regContext & 0x7F;
	...
	case IOCTL_SIOCTL_BUFFERED:
		...
		RtlCopyBytes(outBuf, result4buffer, outBufLength);

		SIOCTL_KDPRINT(("\tData to User :"));
		PrintChars(outBuf, datalen4buffer);
		...
		Irp->IoStatus.Information = (outBufLength<datalen4buffer ? outBufLength : datalen4buffer);
